function Tree(initObj) {
	...
	document.oncontextmenu = function(e) {
		if (typeof initObj.onContext == "function") {
			initObj.onContext(null, function (err, data) {
...
//			}
	...
	function editNodeNameChange() {
		if (idEditing) {
			...
			if (l_data.name != l_strName) {
				...
				l_data.divName.innerHTML = l_data.name
				initObj.onRename(idEditing, l_data.name);
			}
	...
	var divDropTarget = null
	var boolBefore = false
	var g_mouseX = 0
	var g_mouseY = 0

	var divDragWaiting = null
		...
		if (divDragging) {
			...
			if (divDropTarget) {
				...
				if (boolBefore) {
					...
					if (l_divParent) {
						initObj.onDrop(divDragging.id, divDropTarget.id);
